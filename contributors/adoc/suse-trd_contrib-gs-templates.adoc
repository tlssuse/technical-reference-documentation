...
|sliberty |SUSE® Multi-Linux Support (SUSE Liberty Linux)
|suma |SUSE® Multi-Linux Manager
|rancher |SUSE® Rancher Prime
|neuvector |SUSE® Security (NeuVector)
|harvester |SUSE® Virtualization (Harvester)
|longhorn |SUSE® Storage (Longhorn)
...
The default logo is the official SUSE company logo, which is always appropriate for all {trd}.
